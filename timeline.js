 ...
 * jQuery-UI Slider
 */
OpenLayers.Timeline = OpenLayers
    .Class({
      map : undefined,
      slider : undefined,
      ...
      interval : 5,
      speeds : [
          "Really slow", "Slow", "Normal", "Fast", "Really fast"
      ],
        ...
        $(this.slider).slider(
            {
              ...
              change : function(e, ui) {
                if (self.display_layer) {
                  /*
                   * if (self.selectControl) { for (var i in self.map.popups) {
                   * var popup = self.map.popups[i];
                   * self.map.removePopup(popup);
                   * console.log(popup.selectedFeature); var point =
                   * popup.selectedFeature.geometry; console.log(point);
                  ...
                  if (ui) {
                    self.data_format.past_seconds = Math
                        .ceil(self.first + (self.current_date - self.first)
                            * (ui.value / 100.0));
                  }
                  if (!self.cumulative) {
                    self.data_format.lowerlimit = self.data_format.past_seconds
                        - self.timedelta;
                  }
                  else {
                ...
                if (bounds && self.cumulative) {
                  if (self.map.baseLayer.mapObject.getMapTypeId() == GoogleEarth.MAP_TYPE_ID) {
              ...
              slide : function(e, ui) {
                ...
                if (self.current_data) {
                  self.data_format.past_seconds = Math.ceil(self.first
                      + (self.current_date - self.first) * (ui.value / 100.0));
                }
                ...
                var tooltip = $("<div/>").css({
                  ...
                  top : "20px",
                  left : "0px"
                }).addClass("slider-tooltip").text(d);
                $(self.slider).slider().find(".ui-slider-handle").append(
                    tooltip);
                setTimeout(function() {
                  tooltip.remove();
                }, 500);
      ...
      onFeatureSelect : function(feature) {
        ...
        if (feature.attributes.count == 1) {
          ...
          if (feature.attributes.count > 1) {
            desc += "<strong>" + feature.cluster.length
                + " features in this area</strong><br/>";
            desc += "<br/><em>tip: increase the zoom level</em>";
            desc += "</div>";
          }
        var popup = new OpenLayers.Popup.FramedCloud("chicken",
            feature.geometry.getBounds().getCenterLonLat(),
            new OpenLayers.Size(1000, 500), desc, null, true, this.onPopupClose);
          ...
          if (past_seconds - firstdate > 0) {
            $(this.slider)
                .slider(
                    "value",
                    Math
                        .ceil(((past_seconds - firstdate) / (this.current_date - firstdate)) * 100));
          }
        }
      },
